...
class VideoRetrieval(QThread):
    ...
    def run(self):      
        ...
        pages = re.findall(r'"cid":(\d+),"page":(.*?),"from":"vupload","part":"(.*?)"', r) 
        if not pages:
            '''可能是番剧、电影啥的无法解析出来'''
            self.error.emit()
            return
    ...
    def resolve(self, page):
        ...
        quality = 80 # 80： 1080p
        params = 'appkey=%s&cid=%s&otype=json&qn=%s&quality=%s&type=' % (appkey, page[0], quality, quality)
        ...
        ret = list(page[1:]) + [self.title, v_time, size, [info['url']]] # [类型] 序号 分p标题 标题 时长 大小 链接
        threading.Thread(target=self.dumpInfo, args=(self.key, ret)).start()
        self.done.emit(1, ret)
