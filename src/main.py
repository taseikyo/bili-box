...
class MWin(QMainWindow, Ui_MWin):
    ...
    def __init__(self, parent=None):
        ...
        self.lineEditHome.setPlaceholderText("输入视频的链接/av号")
    ...
    def connectSlots(self):
        ...
        self.pathAction.triggered.connect(
            lambda: QDesktopServices.openUrl(QUrl(self.dlpath))
        )

        # 关于菜单下 Action
        self.authorAction.triggered.connect(
            lambda: QDesktopServices.openUrl(QUrl("https://github.com/taseikyo"))
        )
        self.appAction.triggered.connect(
            lambda: QDesktopServices.openUrl(
                QUrl("https://github.com/taseikyo/bili-box")
            )
        )

        # 按钮
        self.searchBtn.clicked.connect(self.resolveInput)
        self.vdlPathBtn.clicked.connect(
            lambda: QDesktopServices.openUrl(QUrl(self.dlpath))
        )
        ...
        self.comboBoxHome.currentIndexChanged.connect(self.setLineEditHint)

    def changePage(self, index):
        """修改当前页"""
        self.stackedWidget.setCurrentIndex(index)

    def setLineEditHint(self, index):
        if index == 0:
            self.lineEditHome.setPlaceholderText("输入视频的链接/av号")
        elif index == 1:
            self.lineEditHome.setPlaceholderText("输入画师的个人链接")
        elif index == 2:
            self.lineEditHome.setPlaceholderText("输入收藏夹链接")
    ...
    def setLogo(self):
        ...
        if os.path.exists(f"cache/{datetime.date.today()}.txt"):
            imgs = os.listdir("images")
            a = random.choice(imgs)
            if a == "background":
                a = "images/e6ee42956e1020c5be0567984d150afc75ada335.png"
            self.logo.setText(link.format(src=f"images/{a}"))
            self.logo.setToolTip("哔哩哔哩 (゜-゜)つロ 干杯~-bilibili")
            return

        open(f"cache/{datetime.date.today()}.txt", "w")
        if 3 <= month <= 5:
            src = "images/bili-spring_1.png"
    ...
    def modifyMinSize(self):
        ...
        text, okPressed = QInputDialog.getText(
            self, "修改窗口尺寸", "输入尺寸:", QLineEdit.Normal, "输入两个数字 以空格分割"
        )
        ...
        if btype == 0:
            ...
            while has_more:
                ...
                for x in items:
                    ...
                    pictures = x["pictures"]
                    print(
                        description, upload_time, view_count, collect_count, like_count
                    )
                    for i in pictures:
                        print(i["img_src"])
        elif btype == 2:
        ...
        if self.vdlAllBox.isChecked():
            self.vtable.setRangeSelected(
                QTableWidgetSelectionRange(
                    0, 0, self.vtable.rowCount() - 1, self.vtable.columnCount() - 1
                ),
                True,
            )
    ...
    def download(self, dtype):
        ...
        if dtype == 0:
            if self.vdlAllBox.isChecked():
                ...
                rows = [
                    x
                    for x in range(self.vtable.rowCount())
                    if self.vtable.item(x, 0).isSelected()
                ]
            if not rows:
                return
            for x in rows[::-1]:
            ...
            if self.pdlAllBox.isChecked():
                ...
                rows = [
                    x
                    for x in range(self.ptable.rowCount())
                    if self.ptable.item(x, 0).isSelected()
                ]
            if not rows:
                return
            ...
            rows = [
                x
                for x in range(self.ftable.rowCount())
                if self.ftable.item(x, 0).isSelected()
            ]
            if not rows:
                return
...
class VideoRetrieval(QThread):
    ...
    def run(self):
        ...
        r = requests.get(url, headers=headers).text
        try:
            title = re.findall(r'<h1 title="(.*?)" class="video-title"', r)[0]
        except Exception as e:
            """视频不见了"""
            self.error.emit()
            return
        ...
        pages = re.findall(
            r'"cid":(\d+),"page":(.*?),"from":"vupload","part":"(.*?)"', r
        )
        if not pages:
            """可能是番剧、电影啥的无法解析出来"""
            self.error.emit()
    ...
    def resolve(self, page):
        ...
        page: (cid, index, page title)
        """
        # 20190225：最新的视频中更新的了无法匹配到 order 导致获取下载链接失败
        """
        ...
        """
        # 参考项目 Henryhaohao/Bilibili_video_download
        ...
        start_url = f"https://www.bilibili.com/video/av{self.key}"
        tmp_headers = {
            "Referer": start_url,  # 注意加上referer
            "User-Agent": "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36",
        }
        r = requests.get(url_api, headers=tmp_headers).json()
        info = r["durl"][0]
        v_time = f"{info['length']//60000}:{info['length']%60000//1000}"
        size = round(info["size"] / 1024 ** 2, 1)
        ret = list(page[1:]) + [
            self.title,
            v_time,
            size,
            [info["url"]],
        ]  # [类型] 序号 分p标题 标题 时长 大小 链接
        threading.Thread(target=self.dumpInfo, args=(self.key, ret)).start()
        self.done.emit(1, ret)

    def dumpInfo(self, aid, info):
        if os.path.exists(f"cache/av{aid}.json"):
            return
        r = {}
        r["aid"] = aid
        r["ptitle"] = info[1]
...
class VideoDownlaod(QThread):
    ...
    def run(self):
        ...
        for i, j in enumerate(self.num):
            t = threading.Thread(
                target=self.download, args=(self.urls[i],), name=str(j)
            )
            threads.append(t)
...
class FavoriteRetrieval(QThread):
    ...
    def dumpInfos(self, info):
        ...
        for x in info:
            if os.path.exists(f"cache/fav{x[0]}.json"):
                continue
            r = {}
...
class FavoriteDownlaod(QThread):
    ...
    def run(self):
        ...
        for i, j in enumerate(self.num):
            t = threading.Thread(
                target=self.download, args=(self.aids[i],), name=str(j)
            )
            threads.append(t)
